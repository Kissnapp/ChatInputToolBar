...
#define MARGIN_WIDTH_BETWEEN_BUTTONS ((DEFAULT_MORE_INPUT_VIEW - 2*MARGIN_HEIGHT - LTZInputToolBarDefaultMutableViewColumns* [LTZMoreInputItemView defaultWidth])/(LTZInputToolBarDefaultMutableViewColumns - 1))
...
- (LTZMoreInputItemView *)itemViewWithIndex:(NSUInteger)index count:(NSUInteger)count item:(LTZMoreInputItem *)item
{
    LTZMoreInputItemView *itemView = nil;
    ...
    CGFloat originX = MARGIN_WIDTH + completePageNum*DEFAULT_MORE_INPUT_VIEW + (index - completePageNum*(LTZInputToolBarDefaultMutableViewRows * LTZInputToolBarDefaultMutableViewColumns) - rowNum * LTZInputToolBarDefaultMutableViewColumns)*([LTZMoreInputItemView defaultWidth] + MARGIN_WIDTH_BETWEEN_BUTTONS);
    
    //3.Get originY for the view that will been showed
    CGFloat originY = rowNum * ([LTZMoreInputItemView defaultHeight] + MARGIN_HEIGHT) + MARGIN_HEIGHT;
    
    //4.Init a LTZMoreInputItemView object for showing
    itemView = [[LTZMoreInputItemView alloc] initWithOriginX:originX OriginY:originY ltzMoreInputItem:item];
